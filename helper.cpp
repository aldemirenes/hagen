...
void Camera::rotateAroundUp(float angleDegree) {
  this->gaze = glm::normalize(glm::rotate(this->gaze, glm::radians(angleDegree), this->up));
}

void Camera::rotateAroundRight(float angleDegree) {
  glm::vec3 rightVec = glm::cross(this->gaze, this->up);
  this->up = glm::normalize(glm::rotate(this->up, glm::radians(angleDegree), rightVec));
  this->gaze = glm::normalize(glm::cross(this->up, rightVec));
}
